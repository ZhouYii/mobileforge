...
func test_battle_enemy_turn_deals_damage() -> void:
	## After player turn, enemy attacks reduce team HP.
	var dungeon_def = _make_dungeon_def()
	_dungeon_runner.start_dungeon(dungeon_def, 1000, 1000)

	var state = _dungeon_runner.get_dungeon_state()
	assert_true(state.is_active, "dungeon should be active after start")
	...
	if cascade_steps.is_empty():
		_board.init_board()
		cascade_steps = MFCascadeResolver.resolve(_board)
	_dungeon_runner.execute_player_turn(cascade_steps, team, team_stats)

	state = _dungeon_runner.get_dungeon_state()
	if state.is_active:
		var hp_before = state.team_hp
		_dungeon_runner.execute_enemy_turn()
		state = _dungeon_runner.get_dungeon_state()
		# Enemy should have attacked and dealt damage
		assert_lte(state.team_hp, hp_before, "enemy turn should reduce or maintain team HP")


func test_battle_wave_progression() -> void:
	## Clearing all enemies advances to next wave.
	var multi_wave_dungeon = MFDungeonTypes.DungeonDef.new({
		"id": 2,
		"name": "Multi Wave Test",
		"stamina_cost": 10,
		"waves": [
			{"enemies": [{"name": "Wave1Enemy", "element": 2, "hp": 100, "atk": 10, "defense": 0, "countdown": 3}]},
			{"enemies": [{"name": "Wave2Enemy", "element": 1, "hp": 200, "atk": 20, "defense": 0, "countdown": 2}]},
		],
		"rewards": {"coins": 500},
	})

	_dungeon_runner.start_dungeon(multi_wave_dungeon, 10000, 10000)
	var state = _dungeon_runner.get_dungeon_state()
	assert_eq(state.current_wave_index, 0, "should start at wave 0")

	var team: Array = [_monster_manager.create_instance(1)]
	var team_stats: Array = [MFMonsterTypes.MonsterStats.new(3000, 5000, 300)]

	# Keep executing turns until wave clears
	var turns := 0
	while state.is_active and state.current_wave_index == 0 and turns < 20:
		var cascade_steps = MFCascadeResolver.resolve(_board)
		if cascade_steps.is_empty():
			_board.init_board()
			turns += 1
	...
	# Should have advanced to wave 1 or won
	assert_true(state.current_wave_index > 0 or not state.is_active,
		"should advance to next wave after clearing enemies")


func test_battle_skill_activation_during_battle() -> void:
	## Activating a skill mid-battle modifies combat state.
	...
	var skill_def = MFSkillTypes.SkillDef.new({
		"id": 100,
		"name": "Test Heal",
		"rules": [{
			"conditions": [{"type": "always_true", "params": {}}],
			"outcomes": [{"type": "heal_flat", "params": {"amount": 5000}}],
		}]
	})

	var context = MFSkillTypes.SkillContext.new()
	context.team = team
	context.team_stats = team_stats
	context.enemies = []
	for enemy in _dungeon_runner.get_dungeon_state().enemies:
		context.enemies.append(enemy)
	context.team_hp = 1000
	context.max_hp = 10000

	var result = _dungeon_runner.activate_skill(skill_def, context)
	assert_eq(result.healing, 5000, "skill should heal 5000 HP")


func test_battle_element_advantage_applied() -> void:
	## Water attacking Fire deals 1.5x damage.
	var chart = MFElementChart.new()
	var combat = MFCombatResolver.new(chart)
	...
	var ctx_dis = MFCombatTypes.DamageContext.new()
	ctx_dis.attacker_element = 2  # Fire
	ctx_dis.defender_element = 1  # Water
	ctx_dis.attacker_atk = 1000.0
	ctx_dis.gems_matched = 3
	ctx_dis.combo_count = 1
	ctx_dis.defender_defense = 0.0

	var result_dis = combat.resolve_player_attack(ctx_dis)

	assert_gt(result_adv.final_damage, result_dis.final_damage,
		"advantaged attack should deal more damage than disadvantaged")
