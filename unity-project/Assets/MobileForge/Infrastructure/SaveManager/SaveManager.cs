...
namespace MobileForge.Infrastructure
{
    ...
    public class SaveManager
    {
        ...
        private readonly Dictionary<string, SaveSegmentEntry> _segments = new();
        ...
        /// <summary>
        /// Optional encryption key. When set, save data is XOR-encrypted.
        /// Empty or null = no encryption (default).
        /// </summary>
        ...
        public bool Save(string slot)
        {
            ...
            if (!string.IsNullOrEmpty(EncryptionKey))
            {
                ...
                var encrypted = SaveFormat.EncryptData(json, EncryptionKey);
                envelope = new Dictionary<string, object> { ["_encrypted"] = encrypted };
            }
        ...
        public bool Load(string slot)
        {
            ...
            if (!string.IsNullOrEmpty(EncryptionKey) &&
                envelope.TryGetValue("_encrypted", out var encObj) && encObj is string encStr)
            {
                var decrypted = SaveFormat.DecryptData(encStr, EncryptionKey);
                try
                {
                    envelope = Newtonsoft.Json.JsonConvert.DeserializeObject<Dictionary<string, object>>(decrypted);
                }
                catch
                {
                    System.Diagnostics.Debug.WriteLine("SaveManager: Decryption/parse failed.");
                    return false;
                }
            }
        ...
        /// <summary>Register a save segment with named saveables and frequency.</summary>
        public void RegisterSegment(string segmentName, List<string> saveableKeys, SaveFrequency frequency = SaveFrequency.Periodic)
        {
            _segments[segmentName] = new SaveSegmentEntry
            {
                SaveableKeys = saveableKeys,
                Frequency = frequency,
                IsDirty = false,
            };
        }

        /// <summary>Mark a specific segment as dirty.</summary>
        public void MarkSegmentDirty(string segmentName)
        {
            if (!_segments.TryGetValue(segmentName, out var seg)) return;
            seg.IsDirty = true;
            if (seg.Frequency == SaveFrequency.EveryChange)
                SaveSegment(segmentName);
        }

        /// <summary>Save only a specific segment.</summary>
        public bool SaveSegment(string segmentName, string slot = "default")
        {
            if (!_segments.TryGetValue(segmentName, out var seg)) return false;
            if (WriteStorage == null) return false;

            var data = new Dictionary<string, object>();
            foreach (var key in seg.SaveableKeys)
                if (_saveables.TryGetValue(key, out var s))
                    data[key] = s.SaveToDict();

            var envelope = SaveFormat.CreateEnvelope(CurrentVersion, data);
            try
            {
                WriteStorage.Invoke($"{slot}_{segmentName}", envelope);
                seg.IsDirty = false;
                return true;
            }
            catch { return false; }
        ...
        private class SaveSegmentEntry
        {
            public List<string> SaveableKeys;
            public SaveFrequency Frequency;
            public bool IsDirty;
        }
    }

    public enum SaveFrequency { EveryChange, Periodic, Manual }
}
